...
Use `p_load()` to load `tidyverse`, `here`, `modelsummary`, `ggstatsplot`, and `inspectdf`.
...
modelsummary(
  ...
  gof_map = c("nobs", "r.squared", "adj.r.squared"), # goodness-of-fit statistics to show
...
In this dataset, we will end up using all the available predictors in the model. But in general, you should not include every measured variable just because it is available.

Importantly, a type of variable called a **mediator** should not be adjusted for. This is a variable that would be on the causal pathway from smoking to FEV, so it is one of the ways smoking might lower lung function. If we adjust for a mediator, we block part of the effect we are trying to measure.

For example, suppose we also measured **chronic cough** (days per week with a productive cough). Cough is a consequence of smoking and can lower FEV, so it lies on the pathway from smoking to FEV:

![Mediator DAG: smoking affects chronic cough, which in turn affects FEV](images/dag_mediator.svg){width=80%}

Adjusting for chronic cough would statistically hold cough constant across smokers and non-smokers, blocking one route by which smoking harms lung function.

**Question:** Label each of these variables as more likely to be a potential confounder or mediator in the context of the smoking effect on FEV. And indicate whether you would include it in the regression model when estimating the smoking effect.

- Neighbourhood of residence: ______
- Parental smoking status: ______
- Frequency of respiratory infections: ______

# Part 4: Prediction from the adjusted model

Our main goal in this workshop was to *explain* the smoking effect, not to build the best possible prediction model. Even so, once we have fitted a regression model, we can also use it to predict FEV for new individuals.

Model 4 has this form:

predicted FEV = intercept + smoking coefficient * smoking + age slope * age + height slope * height + sex coefficient * sex
...
- the raw smoking estimate from Model 1
- the fully adjusted smoking estimate from Model 4
- adjusted R-squared for the final model
...
In this workshop, multiple regression was used as an effect-size estimation tool. You:

1. compared smokers and non-smokers with summaries, `ggbetweenstats()`, and a raw model;
2. added confounders one at a time and compared models with `modelsummary()`;
3. used DAGs to think about confounders versus mediators;
4. predicted FEV from the final adjusted model.
