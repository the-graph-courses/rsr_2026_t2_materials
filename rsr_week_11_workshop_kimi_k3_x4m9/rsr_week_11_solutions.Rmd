...
```{r count-missing-answer}
sum(is.na(burns_raw$co_hemo))
```
...
**Answer:** **93 patients** have a missing carboxyhemoglobin value. When a model includes `co_hemo`, these patients are dropped from the fit, so the model uses a **complete-case** subset of 676 - 93 = **583 patients**.
...
burns <- burns_raw %>%
  ...
  mutate(
    ...
    pf_ratio_group = factor(pfdivide, levels = 0:3,
                            labels = c(">300", "200-300", "100-200", "<100")),
    mech_vent = factor(inhalation_injury %in% c("Upper", "Lower"),
                       levels = c(FALSE, TRUE), labels = c("No", "Yes"))
  ) %>%
  select(mortality, age_years, tbsa_percent, co_hemo,
         inhalation_injury, mech_vent, pf_ratio_group)
...
**Answer:** The proxy is imperfect. It counts some patients who were not ventilated as ventilated (about 40% of the Upper group and 14% of the Lower group), so it really measures something closer to **intubation or airway-injury severity** than ventilation itself. This misclassification means results for the proxy will not match the paper's ventilation rows exactly.
...
**Answer:** `mech_vent` is a perfect function of `inhalation_injury`: it is "Yes" exactly when inhalation injury is Upper or Lower. Its information is therefore already fully contained in the inhalation dummy variables, so the model has **perfect collinearity** and cannot estimate a separate effect for each. R will silently drop one of the terms (we see this in Part 3).
...
**Answer:** Each additional percentage point of carboxyhemoglobin was associated with about **1.031 times the odds** of mortality (about 3.1% higher odds). The 95% CI (0.985 to 1.077) **crosses 1**, and the p-value is 0.178, so the result is **not statistically significant**. This matches the paper's univariable estimate almost exactly (OR 1.031, 95% CI 0.986 to 1.077, p = 0.178).
...
**Answer:** The proxy **misclassifies** some patients: some intubated patients who were never ventilated are counted as ventilated. This kind of misclassification usually **dilutes** the association, pulling the odds ratio toward 1, so the proxy's estimate is weaker than the estimate from the truly measured variable.

**Question 2.3:** Looking at the combined table, which predictors were statistically significant in the univariable models?

Ans 2.3:

**Answer:** Age, TBSA burned, the ventilation proxy, Upper and Lower inhalation injury (vs Normal), and PF ratio <100 (vs >300) were significant. Subjective inhalation injury, PF ratio 200-300 and 100-200, and carboxyhemoglobin were not. This agrees with the paper's univariable findings.
...
**Answer:** The blanks are β₄(**Upper**), β₅(**Lower**), β₇(**PF 100-200**), and β₉(**carboxyhemoglobin**):

> log-odds(mortality) = β₀ + β₁(age) + β₂(TBSA) + β₃(Subjective) + β₄(Upper) + β₅(Lower) + β₆(PF 200-300) + β₇(PF 100-200) + β₈(PF <100) + β₉(carboxyhemoglobin)
...
**Answer:** exp(β₄) is the **adjusted odds ratio** comparing the Upper inhalation group with the Normal group, holding all other predictors constant. Normal and >300 are the **reference categories**: their log-odds are absorbed into the intercept β₀, so they need no separate terms. All the β₃ to β₈ coefficients are comparisons with these reference groups.
...
```{r multivariable-model, eval=FALSE}
...
mv_model <- glm(mortality ~ age_years + tbsa_percent + inhalation_injury +
...
**Answer:** The model uses **583 patients**. The 93 patients with a missing carboxyhemoglobin value are dropped, because a model can only use rows with complete data on all included predictors (complete-case analysis). This is worth flagging as a limitation when reporting.

## The fitted equation and adjusted odds ratios

Display the fitted equation.

```{r multivariable-equation, eval=FALSE}
...
```{r multivariable-equation-answer}
extract_eq(mv_model, use_coefs = TRUE) %>% preview_eq()
```

Now display the adjusted odds ratios with confidence intervals.

```{r multivariable-results, eval=FALSE}
...
```{r multivariable-results-answer}
mv_results <- tidy(mv_model, conf.int = TRUE, exponentiate = TRUE)
mv_results
```
...
**Answer:** Holding TBSA burned, inhalation injury, PF ratio group, and carboxyhemoglobin constant, each additional year of age was associated with about **1.068 times the odds** of mortality (about **6.8% higher odds** per year). This is **stronger** than the univariable estimate of 1.034, so the other predictors were masking part of age's association.
...
**Answer:** When the paper additionally adjusted for mechanical ventilation, the Upper group's odds ratio dropped toward 1 and became non-significant. This suggests that much of the Upper group's strong apparent association operates **through** ventilation (ventilation confounds or mediates the inhalation association). Our model cannot reproduce this directly, because we lack a measured ventilation variable and our derived proxy cannot enter the same model as the inhalation groups.
...
**Answer:** The univariable association of PF ratio <100 with mortality appears to be largely **explained by the other predictors**: sicker patients with low PF ratios also tend to be older, have larger burns, and have inhalation injuries. After adjusting for these, the odds ratio moved toward 1 and became non-significant, consistent with confounding rather than an independent effect. The paper found the same pattern.
...
```{r patient-log-odds-answer}
patient_log_odds <- -9.1864 + (0.0656 * 50) + (0.0953 * 30) + 1.3845 + (0.0890 * 2)
...
**Answer:** Yes, both give approximately **0.185**, or **18.5%**. The unrounded values differ very slightly because the hand calculation uses rounded coefficients.
...
**Answer:** The `mech_ventYes` coefficient is `NA`: R silently **dropped** it. Because `mech_vent` is "Yes" exactly when inhalation injury is Upper or Lower, its dummy is a perfect sum of the Upper and Lower dummies. With **perfect collinearity**, no separate coefficient can be estimated.
...
```{r vent-swap-model-answer}
mv_vent_model <- glm(mortality ~ age_years + tbsa_percent + mech_vent +
                       pf_ratio_group + co_hemo,
                     family = binomial, data = burns)

mv_vent_results <- tidy(mv_vent_model, conf.int = TRUE, exponentiate = TRUE)
mv_vent_results
```
...
**Answer:** Holding the other predictors constant, patients in the ventilation proxy group had about **3.478 times the odds** of mortality (95% CI 1.861 to 6.623, p < 0.001). This matches the paper's conclusion that ventilation is a strong independent risk factor (OR 3.774, p = 0.042), although our proxy's point estimate is somewhat smaller and its confidence interval narrower.
...
```{r compare-aic-answer}
AIC(mv_model, mv_vent_model)
```
...
**Answer:** The nearly identical AICs mean the two parameterizations carry almost the **same explanatory information**: the ventilation proxy is essentially a compressed version of the inhalation groups (it collapses Upper and Lower into one level). The swap model achieves the same fit with fewer parameters, so it is slightly more parsimonious, but neither model is clearly "better" on statistical grounds. The choice should be guided by the clinical question and by which variable is more meaningful to report.
...
**Answer:** **10 parameters**: one intercept, one coefficient each for age, TBSA, and carboxyhemoglobin, three dummy coefficients for inhalation injury, and three for PF ratio group.

**Question 4.2:** Based on the guideline of at least 10 events per parameter, do we have enough events for this model?

Ans 4.2:

**Answer:** Yes. There are **156 deaths** among the 583 complete-case patients, giving 156 / 10 = **15.6 events per parameter**, comfortably above the rule-of-thumb minimum of 10.
...
**Answer:** All VIF values are low (roughly 1.1 to 1.4), with TBSA highest at about 1.4. Values below about 5 (some say 10) are not concerning, so collinearity is **not a problem** here.
...
**Answer:** Considerably more than 5% of the binned residuals fall **outside** the error bounds (only about half are inside), especially at higher fitted probabilities. This suggests the model's shape does not fully capture the data; for example, mortality rises extremely steeply at very high TBSA. One remedy is to check the functional form of the continuous predictors, as we do for age in Part 5.
...
**Answer:** No, this is **not a problem**. Logistic regression does not assume normally distributed residuals. With a binary outcome, each residual is one of only two values (0 minus the fitted probability, or 1 minus the fitted probability), so the Q-Q panel will almost always look badly non-normal no matter how good the model is. `check_model()` is a **generic** wrapper that draws a standard set of panels for many model classes; not every panel applies to every model. The package's own `check_normality()` function says this explicitly for GLMs and recommends simulated residuals instead. The same logic applies to the homogeneity-of-variance panel: binomial variance is fully determined by the fitted probability, so there is no separate constant-variance assumption to check.
...
**Answer:** The simulated residuals are consistent with a **uniform distribution** (p ≈ 0.4), so there is no evidence that the binomial model family is inappropriate for these data.
...
**Answer:** Roughly, yes. The observed log-odds rise fairly steadily from the 40s onward, with a small dip across the 30s and 40s. The overall pattern is close to a straight-line increase, so the linear-in-log-odds assumption looks reasonable for age.

## Fit the three candidate forms

Fit the three models yourself.

```{r age-form-models, eval=FALSE}
...
age_linear_model <- glm(mortality ~ age_years, family = binomial, data = burns)

age_decade_model <- glm(mortality ~ age_decade, family = binomial, data = burns)

age_quad_model <- glm(mortality ~ age_years + I(age_years^2), family = binomial, data = burns)
```

Now compare the three models with AIC.

```{r age-form-aic, eval=FALSE}
...
```{r age-form-aic-answer}
AIC(age_linear_model, age_decade_model, age_quad_model)
```
...
**Answer:** Keep the **linear** form. It has the lowest AIC (742.8) with only 2 parameters. The quadratic form is essentially tied (742.9, a difference well below 2), so its extra parameter buys no meaningful improvement and only makes the model harder to interpret. The decade groups fit worse (748.3 with 7 parameters) and also discard information by cutting a continuous variable. Prior knowledge points the same way: burns severity scores such as ABSI treat age as a simple linear term.
...
**Answer:** The model is linear on the log-odds (link) scale **by construction**: the linear predictor is a straight line by definition. Converting log-odds to probabilities uses the nonlinear inverse-logit (sigmoid) transformation, which bends the straight line into an S-shaped curve.

**Question 5.4:** Why is `geom_step()` the right geom for the decade-group model?

Ans 5.4:

**Answer:** The decade model assigns **one constant fitted value to everyone within a band**, so its predictions stay flat within each band and jump only at the band boundaries. That staircase shape is exactly what `geom_step()` draws; a smooth line would wrongly suggest the estimate changes gradually within a band.
...
**Answer:** In a multivariable logistic regression model adjusting for all other predictors, each additional year of age was associated with **6.8**% higher odds of mortality (OR **1.068**, 95% CI 1.046 to 1.091), and each additional percentage point of TBSA burned was associated with **10.0**% higher odds (OR **1.100**, 95% CI 1.083 to 1.120). Compared with Normal inhalation status, the Upper group had OR **3.993** (95% CI 1.635 to 9.945) and the Lower group had OR **2.369** (95% CI 1.091 to 5.163). Compared with PF ratio >300, the <100 group had OR **1.547** (95% CI 0.645 to 3.736), which **was not** statistically significant. Each additional percentage point of carboxyhemoglobin had OR **1.093** (95% CI 0.995 to 1.183).
...
## Challenge facilitator notes

Running `scripts/birthwt_regression_table_example.R` produces `scripts/birthwt_regression_table.png`, a table in the style of the example image. Points to check when students use an LLM for this task:

- The raw n/N (%) values should match simple `count()`/`tabyl()` summaries, not model output.
- Continuous predictors should contribute **one** odds ratio per unit increase, even though categories are shown for the raw proportions.
- Reference categories should display "Reference" (or OR = 1) rather than an estimated OR.
- All ORs and CIs should match `tidy(model, conf.int = TRUE, exponentiate = TRUE)` output.
