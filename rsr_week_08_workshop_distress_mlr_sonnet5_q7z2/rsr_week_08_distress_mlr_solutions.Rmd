---
title: "SOLUTIONS: Replicating a publication's multiple linear regression table"
...
These are the completed solutions for the week 8 exercise.
...
Answer: It became `epidemic_related_dreams` (the hyphen is converted to an underscore and everything is lowercased).
...
Answer: 61 rows become `NA`. This is because the raw `marriage` column has a third code, 2, which is not defined in the codebook (it may mean "divorced" or something similar), and it falls outside the two levels (`0`, `1`) we told `factor()` to expect, so those rows become missing.
...
Answer: Only `marriage` has missing values (61 `NA`s), from the undefined code 2 discussed above. Since `lm()` drops any row with a missing value in any model variable, 61 rows will be excluded from the regression, leaving 1599 - 61 = 1538 rows.
...
```{r fit-model}
distress_model <- lm(
  total_scores_of_k6 ~ marriage + history_of_visiting_sea + changes_over_living_situations +
    emotional_control + epidemic_related_dreams + coping_style,
  data = model_data
)
...
Answer: 1538 observations were used (the summary output reports "(61 observations deleted due to missingness)"). This matches the prediction from the previous question.

**Question**: What is the model's R-squared? How does it compare with the paper's reported R-squared of 0.862?

Answer: Our R-squared is about 0.863, which is essentially the same as the paper's reported 0.862.

## Step 5: Get confidence intervals

```{r model-ci}
confint(distress_model)
```
...
Answer: Our `marriagemarried` coefficient is about -0.660, in the same direction as the paper's -0.588 (married people have lower predicted distress than unmarried people, the reference group in both cases), and reasonably close in size, though not identical.
...
Answer: `emotional_control` (about 1.972 vs. 1.995), `epidemic_related_dreams` (about 1.603 vs. 1.597), and `changes_over_living_situations` (about 1.057 vs. 1.040) are all very close to the paper's values. `coping_style` (about -2.169 vs. -2.135) and `history_of_visiting_sea` (about 1.401 vs. 1.449) are also close. `marriage` is the furthest off (-0.660 vs. -0.588), most likely because we cannot be sure the paper handled the undefined `marriage` code 2 the same way we did.
...
Answer: No. A very small p-value only tells us that we can be very confident the true coefficient is not exactly zero; it says nothing about how large or practically meaningful the effect is. With a large sample size like this one (n = 1538), even small, practically unimportant effects can produce very small p-values. To judge practical importance we should look at the size of the coefficient (and its confidence interval) relative to the outcome's scale (K6 ranges 0-24), not just the p-value.
...
Answer: Yes. Our `coping_style` (positive vs. negative) coefficient is negative (about -2.169), meaning that positive copers have lower predicted distress than negative copers, the reference group. Equivalently, negative copers have higher predicted distress, which is consistent with the paper's finding and with the sign of their reported coefficient (-2.135), even though the paper's coding direction for the reference group is not stated as explicitly as ours.
...
Answer: A longitudinal (prospective) design would be needed, one that measures coping style (or assigns/observes a change in coping style) at an earlier time point and then measures distress at a later time point, ideally while also accounting for baseline distress. An experimental design, such as a randomized trial of a coping-style intervention with distress measured afterward, would give the strongest causal evidence, though that is often impractical for something like coping style.
...
Answer: Yes. Difficulty controlling one's emotions could just as easily be a *symptom* of high distress rather than a cause of it; someone who already feels hopeless or nervous (K6 symptoms) may naturally report more difficulty controlling their emotions. Similarly, epidemic-related dreams could be a *consequence* of already being highly distressed or anxious (distress disrupting sleep and producing anxious dreams), rather than the dreams themselves being an independent cause of the distress score.
...
Answer: Yes. "Difficulty controlling emotions" is conceptually very close to several K6 items (feeling nervous, restless, hopeless), and "frequency of distressing dreams" is a recognized symptom of trauma-related psychological distress, closely related to, rather than clearly separate from, the K6 construct. When a "predictor" is really just a different way of asking about the same underlying construct as the outcome, the resulting high R-squared and large, highly significant coefficients partly reflect this overlap (sometimes called criterion contamination) rather than a genuinely separate, informative predictive relationship. This is a likely explanation for why this model's R-squared (0.86) is so much higher than is typical for social-survey regressions.
...
Answer: For example: "In this cross-sectional survey, a negative coping style was associated with higher K6 distress scores, holding the other modeled variables constant; because coping style and distress were measured at the same time, this association may reflect coping style influencing distress, distress influencing coping style, or both, and causal direction cannot be determined from this design."
...
Answer: We were able to closely replicate five of the six coefficients in Table 4 (history of visiting Wuhan, changes over living situations, emotional control, epidemic-related dreams, and coping style), and R-squared matched almost exactly (0.863 vs. 0.862). The `marriage` coefficient was in the right direction but somewhat different in size, likely due to how the undefined `marriage` code was handled. Despite the strong statistical fit, we should not treat this model as establishing that these six factors *cause* distress: the data are cross-sectional, several predictors (especially coping style, emotional control, and epidemic-related dreams) are plausibly reverse-caused by distress or conceptually overlapping with it, and the variable-selection process (stepwise regression) is itself statistically unstable. The paper's underlying descriptive finding, that distress was higher among the unmarried, those who had visited Wuhan, and those reporting more negative perceived impacts and coping, is a reasonable, well-supported association, but the causal language used in the paper's abstract and discussion goes beyond what a cross-sectional survey can support.
