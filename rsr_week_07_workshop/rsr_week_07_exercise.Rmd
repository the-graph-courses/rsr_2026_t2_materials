...
**Question**: In `summary(sex_model)`, which sex is the reference group? In one sentence, what does the `sexMale` coefficient tell you?
...
A one-way ANOVA does not report those individual comparisons. It asks one **overall** question: is there any difference among the group means? That overall test comes from the regression too. Scroll to the bottom of `summary(grade_model)` for the model's F statistic and p-value, which test whether the set of grade coefficients are all zero together.
