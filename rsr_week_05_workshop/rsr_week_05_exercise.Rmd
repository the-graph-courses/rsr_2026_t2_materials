...
-  rename variables to make them more readable, since they will be presented in the paper's table
...
survey <- survey %>%
  mutate(
    ...
    `History of visiting Wuhan` = _______(
...
Call the tabyl function to check the four categorical variables of Gender, Age category, Marriage, and History of visiting Wuhan before continuing.

```{r group-checks}
tabyl(survey, Gender)
tabyl(survey,`Age category`)
tabyl(survey,________) # Marriage
tabyl(survey,________) # History of visiting Wuhan
```
...
group_stats <- survey_long %>%
  ...
  summarise(
    ...
    ci_hi = _______ + qt(0.975, df = n - 1) * se,
    .groups = "drop"
  ) %>%
  # arrange the data by variable and group to match the order of the table
  mutate(
    variable = factor(
      variable,
...
Tip: variable names that contain spaces, like `Age category` and `History of visiting Wuhan`, must be wrapped in backticks when you use them in a formula.
...
Practice using LLMs for formatting by asking for a version of the table that adds an in-cell bar for the mean K6 score.
...
Then write a prompt that includes the object name, the column containing the means, and the package you want to use.
