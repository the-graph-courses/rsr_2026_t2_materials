...
| Binary outcome (e.g. died/survived, anaemic/not anaemic) | **Logistic regression** | odds ratio, 95% confidence interval, p-value, AIC |
...
Run `check_model()` on your final multivariable model and present and describe the results. 
...
-   Is the dataset from the provided collection, or an external dataset approved by an instructor?
-   Are an appropriate outcome and predictors selected for the research purpose?
-   Is there a focused and meaningful research question and hypothesis?
-   Does the report justify why this research question and hypothesis are important?
-   Does the report clearly align with the chosen research purpose (either comparing predictors or estimating a causal effect)?
-   Is the choice of the main analytic approach (linear vs logistic regression) appropriate for the outcome?

### Methods (10 points)

-   In the methods section, is the data source described accurately, including the study design, population, and unit of observation?
-   Are the outcome and predictors clearly defined, including coding and reference groups where relevant?
-   Is the choice of predictors or covariates justified using subject-matter knowledge?
-   Are data cleaning, exclusions, and missing-data handling clearly described?
-   Are the descriptive analysis, univariable models, multivariable model, and diagnostics described in enough detail for another analyst to reproduce the analysis?

### Descriptive analysis (10 points)

-   In the results section, is the analytic sample size, including any exclusions, reported accurately?
-   Is there a nicely-formatted Table 1 that describes the analytic sample, using mean and SD for continuous variables and number and percentage for categorical variables.
-   Is there at least one well-designed and informative figure that shows the outcome distribution or its relationship with an important predictor?
-   Are the tables and figures generated with R code and are clearly labelled and presented?
-   Are the descriptive findings interpreted accurately and concisely in the text?

### Univariable regression (15 points)

-   In the results section, is a separate univariable model correctly fitted for each planned predictor?
-   Are the beta coefficients or odds ratios reported appropriately with 95% confidence intervals?
-   Are the main findings for selected predictors interpreted correctly in the text, including the sizes of associations, units, and reference groups where relevant?
-   Are the statistical findings distinguished from clinical or practical importance?

### Multivariable regression and comparison (25 points)

-   In the results section, is one multivariable model correctly fitted with at least three planned predictors?
-   Are the key coefficients interpreted using appropriate conditional language?
-   Is the model fit calculated and reported appropriately using R-squared and adjusted R-squared for linear regression, or AIC for logistic regression?
-   Is there a table (or multiple tables) presenting univariable and multivariable estimates?
-   Does the report explain how the main coefficient or odds ratio changed after adjustment.
-   Does the report identify which predictors remained associated after mutual adjustment?
-   Does the report discuss plausible reasons for changes between the univariable and multivariable results, such as confounding or shared information between predictors?

### Diagnostics, discussion, and limitations (15 points)

-   Is the `performance::check_model()` function run on the final multivariable model, and are the diagnostic findings well-presented and interpreted?
-   Are any important diagnostic concerns reported honestly and included as limitations?
-   Does the discussion directly answer the research question and state whether the findings support the hypothesis?
-   Does the discussion discuss the scientific or practical meaning of the findings?
-   Are at least two relevant limitations identified?
-   Are the conclusions appropriate for the study design, analysis, and chosen research purpose, without unwarranted causal claims?

### Reproducible R and professional communication (10 points)

-   Are all required sections present: Introduction, Methods, Results & Interpretation, Discussion & Limitations, and References?
-   Is the data source cited in the References section?
-   Are there at least 750 words of narrative text (excluding code, code comments, and references)?
-   Does the code run without errors; all outputs are generated in R?
-   Is the code clear and reproducible, with relative file paths such as `here::here()`?
-   Are the code chunks hidden (`echo = FALSE`); messages and warnings suppressed?
-   Are the tables and figures readable, clearly labelled, and include units where relevant?
-   Does the report knit without errors after restarting R and knitting from a clean session?
-   Is the source file correctly named, and both the `.Rmd` (or `.qmd`) source and knitted `.html`, `.docx`, or `.pdf` report are ready to upload?
-   Is the prose clear, proofread, written mainly in the student's own words, and scores under 50% AI-generated text on [Pangram](https://www.pangram.com)? NOTE: This is a pass/fail criterion.
...
+-----------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------+
| Fit a multivariable logistic model, merge tables, use AIC, check binned residuals | Week 11 workshop                                                                                                      |
+-----------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------+
...
#| fig-width: 7
#| fig-height: 4.5
binned_res <- binned_residuals(multivar_glm)
binned_res
plot(binned_res)
```

The binned residuals plot shows quite some deviations from the error bounds. This should be reported as a limitation in the Discussion & Limitations section.