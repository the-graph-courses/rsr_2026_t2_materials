...
### Introduction, data selection and research aim: 15 points

-   Is the dataset an individual-level dataset for which the regression approaches taught in the course are appropriate?
-   Does the report use an appropriate outcome and set of predictors?
...
-   Is the data source, including collection method and study design, decently well described? (If the report reuses data from a published paper, this description can only be as detailed as the original paper.)
...
-   Where relevant, are data cleaning, exclusions, and missing-data handling clearly described?
-   Are the descriptive analysis, regression procedures and model diagnostics described in enough detail for another analyst to reproduce the analysis?
...
-   Is there at least one table that describes the final sample, using, for example, mean and standard deviation to describe continuous variables and percentage compositions to describe the categorical variables?
...
-   Are the tables well-designed, mistake-free, and appropriately labelled?
-   Are the descriptive findings, including some of what was presented in the table(s) above, interpreted accurately in the text?

### Univariable regression: 15 points

-   Is there a correctly-fit univariable model for each predictor that was mentioned in the methods?
-   Is there at least one table presenting univariable estimates, alone or in conjunction with multivariable estimates?
...
-   In the results section, is there at least one multivariable model appropriately fitted with at least three of the predictors planned?
-   When multivariable associations are presented, is appropriate conditional language used?
-   Is the model fit reported using R-squared or related measures for linear regression, and AIC for logistic regression when models are being compared?
-   Is there a table (or multiple tables) presenting the multivariable estimates?
-   For predictor-comparison reports, does the report identify which predictors remain associated after adjustment?
-   If there are important changes between the univariable and multivariable results, does the report discuss these, including plausible reasons for such changes?
-   If relevant, are the statistical findings distinguished from clinical or practical importance (either for univariable or multivariable models)?

### Diagnostics, discussion, and limitations: 15 points

-   Are diagnostics run on the final multivariable model, and are the findings well presented? (`performance::check_model()` is one good option. For logistic models, a residual or binned-residual check may be enough.)
-   If there are diagnostic concerns, are they addressed in the analysis or reported as limitations?
-   Does the discussion directly answer the research question?
...
-   Is the code reproducible? That is, is it written so that rerunning it would likely produce the reported results, including the use of relative file paths such as `here::here()`?
